...
- **Mailboxes (Left)**: Each source is an account header in the sidebar outline.
  - **Local sources**: Pages (expanding to nested trunk subfolders from `graph.parent`), Outputs, Publish, Plan, Activity, and Content Audit — plus **Recipes** when the built graph carries Cooklang (`recipe` facet) nodes.
  - **GitHub sources**: Pages and trunk subfolders, Outputs, Publish, Plan, Activity, Content Audit, plus **Remote** (branch status, ahead/behind tracking, commit/push/sync), **Issues** (open repository issues), and **Pull Requests** (open PRs and authoring).  - Selecting an account header opens that source's Pages mailbox.
...
- **Recipes**: When the built graph carries Cooklang nodes (a `recipe` facet on ≥1 node), the Recipes mailbox lists every recipe with its ingredient count. Selecting a row reads that page in the shared reading pane; the drawer's Recipe Scale section scales it (`boris recipe-scale` runs only on factor change, never for the list). Markdown-only sources never see this row.
